...
LORA_MANAGER = LoRAManager(merge_weights=True)
...
class LoraLinear(lora.LoRALayer, nn.Module):
    ...
    def __init__(
        ...
        if r > 0:
            ...
            self._lora_A, self._lora_B = None, None
            ...
            if mode and self.merged:
                ...
                if self.r > 0:
                    if self._lora_A is not None and self._lora_B is not None:
                        ...
                        self.lora_A = nn.Parameter(self._lora_A)
                        self.lora_B = nn.Parameter(self._lora_B)
                        # self.reset_parameters()
                ...
                if self.r > 0:
                    self.weight.data += T(self.lora_B @ self.lora_A) * self.scaling
                    self._lora_A, self._lora_B = self.lora_A.data, self.lora_B.data
                    delattr(self, "lora_A")
                    delattr(self, "lora_B")
                self.merged = True
